function f = funEval2(param, delta_t, period, Q_func)
omiga = param(1);
time_delta = 100 - param(2);
threshold = param(3);
...
%     end
    ...
    %%%%%%%%%% 对高压管 %%%%%%%%%%
    % 减压阀
    if gaoYaPipe_P > threshold
        QlowPress = Q_func(gaoYaPipe_P - 0.5, rho(ind));
    else
        QlowPress = 0;
    end
    Qout1 = calOutQ1(mod(t, period_out), P0(ind)-0.1, rho(ind), Q_function);
    Qout2 = calOutQ1(mod(t+time_delta, period_out), P0(ind)-0.1, rho(ind), Q_function);
    Qout = Qout1 + Qout2 + QlowPress;
...
f = mean(abs(P0(end-5000:end-1)-100));
end